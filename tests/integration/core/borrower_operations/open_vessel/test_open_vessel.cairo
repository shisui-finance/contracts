...
fn given_normal_condition_should_open_vessel() {
    let (
        ...
        setup();
    borrower_operations.contract_address.print();

    let collateral_address = contract_address_const::<'ETH/USD'>();
    ...
    start_prank(CheatTarget::All, borrower_operations.contract_address);

    // open vessel with min debt of 2000. So 1.5 ETH = 2400 debt token
    borrower_operations
        .open_vessel(
            collateral_address,
            1_500000000000000000,
            2400_000000000000000000,
            upper_hint_address,
            lower_hint_address
        );
}
