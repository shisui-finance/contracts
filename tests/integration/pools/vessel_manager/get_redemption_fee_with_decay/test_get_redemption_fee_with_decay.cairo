...
fn when_base_rate_is_defined_should_return_redemption_fee() {
    ...
    store(
        ...
        map_entry_address(
            selector!("last_fee_operation_time"), array![asset.contract_address.into()].span()
        ),
        array![10].span()
    );

    let fee = vessel_manager.get_redemption_fee_with_decay(asset.contract_address, 10);
...
fn when_base_rate_is_defined_redemption_fee_is_below_asset_draw_should_revert() {
    let (
        borrower_operations,
        vessel_manager,
        adress_provider,
